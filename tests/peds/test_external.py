...
def _expected_delta(joint: str, angle: float) -> np.ndarray:
    """The bone-local delta external.py builds for a pushed wire angle (single-target joints)."""
    targets = BONE_MAP[joint]
    assert targets is not None
    (target,) = targets
    return axis_angle_quat(target.axis, target.sign * target.scale * angle)


def _provider(
    joint_order: tuple[str, ...],
    neutral_rotations: np.ndarray | None = None,
    neutral_translations: np.ndarray | None = None,
    **kwargs: float,
) -> ExternalPoseProvider:
    if neutral_rotations is None:
        neutral_rotations = np.tile(np.array([0.0, 0.0, 0.0, 1.0]), (len(joint_order), 1))
    if neutral_translations is None:
        neutral_translations = np.zeros((len(joint_order), 3))
    return ExternalPoseProvider(joint_order, neutral_rotations, neutral_translations, **kwargs)
...
def test_full_path_joint_order_matches_bone_map_leaves() -> None:
    ...
    provider = _provider(joint_order)

    angle = math.pi / 4.0
    provider.push(0.0, ["l_knee"], [angle])
    pose = provider.evaluate(sim_time=0.0, dt=1.0)  # dt >> blend_s snaps weight to 1.0

    np.testing.assert_allclose(pose.rotations[2], _expected_delta("l_knee", angle), atol=1e-9)


def test_unmapped_bone_holds_neutral() -> None:
    joint_order = ("Hips", "LeftLeg")  # Hips is not a BONE_MAP target
    neutral_hips = axis_angle_quat((0.0, 0.0, 1.0), 0.4)
    rotations = np.stack([neutral_hips, np.array([0.0, 0.0, 0.0, 1.0])])
    translations = np.array([[0.0, 0.0, 0.9], [0.0, 0.1, -0.4]])
    provider = _provider(joint_order, rotations, translations)

    provider.push(0.0, ["l_knee"], [math.pi / 2.0])
    pose = provider.evaluate(sim_time=0.0, dt=1.0)

    np.testing.assert_allclose(pose.rotations[0], neutral_hips, atol=1e-9)
...
def test_mapped_bone_composes_onto_neutral() -> None:
    joint_order = ("LeftLeg",)
    neutral = np.array([axis_angle_quat((0.0, 1.0, 0.0), 0.6)])  # non-identity neutral pose
    provider = _provider(joint_order, neutral)
    ...
    assert not np.allclose(override, delta, atol=1e-3)  # distinct from an identity-seeded compose


def test_no_pushes_yields_neutral_stance() -> None:
    joint_order = ("LeftLeg",)
    neutral = np.array([axis_angle_quat((1.0, 0.0, 0.0), 0.2)])
    translations = np.array([[0.0, 0.0, 0.45]])
    provider = _provider(joint_order, neutral, translations)

    pose = provider.evaluate(sim_time=3.0, dt=0.05)
...
def test_staleness_decays_to_neutral_then_ramps_back_on_fresh_push() -> None:
    joint_order = ("LeftLeg",)
    provider = _provider(joint_order, staleness_s=0.5, blend_s=0.3)
    ...
    pose = provider.evaluate(sim_time=0.75, dt=0.15)  # weight ramps 0.5 -> 0.0, pure neutral now
...
def test_blend_hemisphere_continuity_across_weight_ramp() -> None:
    ...
    assert float(np.dot(identity, delta)) < 0.0  # neutral and raw override land in opposite hemispheres
...
def test_waist_distributes_over_spine_chain() -> None:
    joint_order = ("LowerBack", "Spine", "Spine1")
    provider = _provider(joint_order)

    angle = 0.6
    provider.push(0.0, ["waist"], [angle])
    pose = provider.evaluate(sim_time=0.0, dt=1.0)

    targets = BONE_MAP["waist"]
    assert targets is not None
    assert sum(t.scale for t in targets) == pytest.approx(1.0)
