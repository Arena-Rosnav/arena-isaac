...
from peds.providers.gait import GaitProvider
...
try:
    ...
    ExternalPoseProvider = None
    carb.log_warn("peds: peds.providers.external unavailable, joint_state overrides ignored")


_PHYSICS_CALLBACK = "peds/tick"
_CLIP_KEYS = ("walk", "idle")


class PedRuntime:
    """Owns the registry, the shared writer, and the per-ped provider handles."""

    def __init__(self, world: World) -> None:
        self._world = world
        self._writer = make_writer()
        self._registry = PedRegistry(self._writer)
        self._gaits: dict[str, GaitProvider] = {}
    ...
    def spawn(self, name: str, position: np.ndarray, orientation: np.ndarray, model_source: str) -> None:
        ...
        stride = float(clip_meta["walk"]["stride_length_m"]) if "walk" in clip_meta else 1.0

        clips: dict[str, Clip] = {}
        for key in _CLIP_KEYS:
            clip_path = cache_dir / "clips" / f"{key}.usda"
            if clip_path.exists():
                clips[key] = Clip.load(str(clip_path))
        if "idle" not in clips:
            raise ValueError(f"peds: cache {cache_dir} is missing an idle clip")
        joint_order = clips.get("walk", clips["idle"]).joint_order

        gait = GaitProvider(clips, stride_length=stride)
        provider = gait
        external = None
        if ExternalPoseProvider is not None:
            neutral = clips["idle"].rotations[0]
            external = ExternalPoseProvider(gait, tuple(joint_order), neutral_rotations=neutral)
            provider = external
        ...
        self._author_animation(prim_path, joint_order, clips)
    ...
    def update(
        ...
        """Dead-reckoned per-tick command plus gait state, and external joint override."""
        ...
        gait = self._gaits.get(name)
        if gait is not None:
            gait.set_state(int(animation_state), float(math.hypot(velocity[0], velocity[1])))
    ...
    def delete(self, name: str) -> bool:
        ...
        self._gaits.pop(name, None)
        self._externals.pop(name, None)
        delete_prim(ped.prim_path)
        return True

    def _author_animation(self, prim_path: str, joint_order: tuple[str, ...], clips: dict[str, Clip]) -> None:
        ...
        rest = clips.get("idle", clips[next(iter(clips))]).translations[0]
        anim.CreateTranslationsAttr(
            Vt.Vec3fArray([Gf.Vec3f(float(t[0]), float(t[1]), float(t[2])) for t in rest])
        )
