"""External skeletal-pose override provider, driven by pushed ROS4HRI joint angles.

Blends over a fallback PoseProvider (typically the ped's GaitProvider/idle pose):
bones mapped by BONE_MAP follow the external stream absolutely, the pushed angles compose
onto a neutral standing pose (the idle clip's first frame, NOT the skeleton rest, which is
a T-pose on CMU rigs) rather than the fallback's sampled rotation, so a mapped bone stays
independent of the fallback clip. Unmapped bones pass the fallback pose through untouched.
Falls back fully when the external stream goes stale, ramping the blend weight so there is
no visual pop. Pure numpy, importable without carb/omni/pxr (carb is only touched lazily,
...
from peds.providers.base import DistanceDrivenPose, JointPose, ResettablePhase
from peds.providers.bone_map import BONE_MAP
from peds.providers.math import quat_normalize, quat_slerp

if TYPE_CHECKING:
    from peds.providers.base import PoseProvider
...
class _AngleRingBuffer:
    ...
    Stores angles rather than quats (unlike StampedPoseBuffer): the raw arena_people_msgs
    contract is scalar joint angles, and the per-bone axis/sign to turn them into quats is
...
class ExternalPoseProvider:
    """Overrides a fallback PoseProvider's bones with externally-pushed ROS4HRI joint angles."""

    def __init__(
        self,
        fallback: PoseProvider,
        joint_order: tuple[str, ...],
        neutral_rotations: np.ndarray | None = None,
        maxlen: int = 32,
        ...
        if neutral_rotations is None:
            self._neutral_rotations = np.tile(np.array([0.0, 0.0, 0.0, 1.0], dtype=float), (len(joint_order), 1))
        else:
        ...
        schema_sign: list[float] = []
        for name, target in BONE_MAP.items():
            if target is None:
                continue
            ...
            schema_sign.append(target.sign)
        ...
        self._schema_sign: np.ndarray = np.asarray(schema_sign, dtype=float)
    ...
    def evaluate(self, sim_time: float, dt: float) -> JointPose:
        """Sample the fallback pose, then blend mapped bones toward neutral-seeded external angles."""
        fallback_pose = self._fallback.evaluate(sim_time, dt)
        ...
        rotations = fallback_pose.rotations.copy()
        ...
        for k in range(self._schema_bone_idx.shape[0]):
            ...
            delta = axis_angle_quat(axis, float(self._schema_sign[k] * angles[k]))
        ...
        if touched:
            ...
            rotations[idx_arr] = quat_slerp(rotations[idx_arr], overridden[idx_arr], self._weight)

        return JointPose(rotations=rotations, translations=fallback_pose.translations)

    def advance(self, distance: float, dt: float) -> None:
        if isinstance(self._fallback, DistanceDrivenPose):
            self._fallback.advance(distance, dt)

    def reset_phase(self) -> None:
        self._buffer.clear()
        self._weight = 0.0
        if isinstance(self._fallback, ResettablePhase):
            self._fallback.reset_phase()
